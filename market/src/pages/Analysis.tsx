...
const Analysis = () => {
  ...
  // Format stock data for the heatmap (treemap) with null checks
  ...
  // Format cryptocurrency data for analysis with null checks
  const cryptoData = mockCryptos
    .filter(crypto => crypto && typeof crypto.marketCap === 'number' && typeof crypto.price === 'number')
    .map(crypto => ({
  ...
  // Format historical data for charts with validation
  const btcHistoryData = btcHistory
  ...
  const ethHistoryData = ethHistory
    .filter(price => typeof price === 'number' && !isNaN(price))
    .map((price, index) => ({
      day: index + 1,
      price: price || 0
  ...
  // Custom content for the treemap with safe value handling
              ...
              <Card key={crypto.symbol} className="bg-card">
                ...
                <CardContent>
                  <div className="text-2xl font-bold">
                    ${crypto.price < 1 ? (crypto.price || 0).toFixed(4) : (crypto.price || 0).toFixed(2)}
                  </div>
